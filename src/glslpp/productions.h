...
typedef ParserT<GLSLTokenID> ParserTGLSL;

class ParserGLSL : public ParserTGLSL
{
public:
    ParserGLSL(const char* _stringToLex);

protected:
    virtual ProductionT<GLSLTokenID> StartSymbol() const;
};

#define DeclareProduction(_name) \
        extern Node* _name(ParserTGLSL* _p, TokenList<GLSLTokenID>* _tl)

// ------------------------------------------------------------------------------------------------
#define DefineProduction(_name) \
    Node* _name(ParserTGLSL* _p, TokenList<GLSLTokenID>* _tl)

// ------------------------------------------------------------------------------------------------
#define SemanticAction [](ParserTGLSL* _p, TokenList<GLSLTokenID>* _tl, const int) -> Node*

// ------------------------------------------------------------------------------------------------
#define DefineStartSymbol(_ss) ProductionT<GLSLTokenID> ParserGLSL::StartSymbol() const { return _ss; }
